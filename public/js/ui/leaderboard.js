...
export function renderLeaderboard(data, append = false, currentSession = null) {
    ...
    if (currentSession && !append) {
        ...
        li.innerHTML = `
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-6 z-10 w-full mt-5">
                ...
                <div class="flex items-center justify-between md:justify-end md:ml-auto gap-3 md:gap-4 w-full md:w-auto pl-1 md:pl-0">
                    <div class="flex flex-col items-start md:items-end">
                        <span class="font-mono font-black text-xl md:text-3xl text-yellow-300 drop-shadow-[0_0_10px_rgba(250,204,21,0.6)] leading-none" title="Smash Score">${currentSession.smash_score != null ? currentSession.smash_score.toLocaleString() : '---'}</span>
    ...
    data.forEach((player, index) => {
        ...
        li.innerHTML = `
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-6 z-10 w-full">
                <div class="flex items-center gap-3 md:gap-4 min-w-0">
                    <div class="flex items-center justify-center min-w-[3rem] shrink-0">${medalHtml}</div>
                    <div class="flex flex-col min-w-0">
                ...
                <div class="flex items-center justify-between md:justify-end md:ml-auto gap-3 md:gap-4 w-full md:w-auto pl-1 md:pl-0">
                    <div class="flex flex-col items-start md:items-end">
                        <span class="font-mono font-black text-xl md:text-3xl ${playerCheater ? 'text-red-500/40 line-through decoration-red-500/60' : `text-${themeColor}-500 group-hover:text-${themeColor}-400`} transition-colors leading-none" title="Smash Score">${player.smash_score != null ? parseInt(player.smash_score).toLocaleString() : '---'}</span>
                        <div class="flex items-center gap-2 flex-wrap mt-1">
