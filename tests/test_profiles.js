...
(function testMemeCategories() {
    ...
    result = ProfileEngine.analyze([azKeys], 'classic');
    ...
    result = ProfileEngine.analyze([piKeys], 'classic');
    ...
    result = ProfileEngine.analyze([fibKeys], 'classic');
    ...
    result = ProfileEngine.analyze([palindromeKeys], 'classic');
    ...
    result = ProfileEngine.analyze([consoleLogKeys], 'classic');
    ...
    result = ProfileEngine.analyze([loremKeys], 'classic');
    ...
    result = ProfileEngine.analyze([gitKeys], 'classic');
    ...
    result = ProfileEngine.analyze([typeWriterKeys], 'classic');
...
(function testNewStatProfiles() {
    ...
    let result = ProfileEngine.analyze(makeHistory(keys, 2), 'classic');
    ...
    result = ProfileEngine.analyze(makeHistory(keys, 2), 'classic');
    ...
    result = ProfileEngine.analyze(makeHistory(keys, 1), 'classic');
    titles = result.profiles.map(p => p.title);
    ...
    result = ProfileEngine.analyze(history, 'classic');
    titles = result.profiles.map(p => p.title);
...
(function testFunPatternProfiles() {
    ...
    let result = ProfileEngine.analyze(makeHistory(keys, 1), 'classic');
    let titles = result.profiles.map(p => p.title);
    assert(titles.includes('The Alphabet Tourist'), 'All 26 letters pressed → The Alphabet Tourist');

    keys = ['F1', 'F2', 'F3', 'F5'];
    result = ProfileEngine.analyze([keys], 'classic');
    titles = result.profiles.map(p => p.title);
    ...
    result = ProfileEngine.analyze(makeHistory(keys, 8), 'classic'); // ~360 keys over 45 ticks (4.5s) = high KPS, no single tick too big
    titles = result.profiles.map(p => p.title);
