const BaseProfileEngine = require('./BaseProfileEngine');
const { rows, vowels, fKeys, punctuation, deletes, enters, modifiers, copyPasteKeys, mathOperators, konamiCode, wasdZone, numpadZone, arrowZone, mobaZone, vimZone, osZone, mediaZone, dpadFaceZone } = BaseProfileEngine;
...
class ClassicProfileEngine extends BaseProfileEngine {
    static analyze(keyHistory) {
        return super.analyze(keyHistory, PROFILES, 100, 5);
    }

    // Dynamically computed unique profile titles
    static TOTAL_PROFILES = Array.from(new Set(PROFILES.map(p => p.title)));

    static getTotalCount() {
        return ClassicProfileEngine.TOTAL_PROFILES.length;
    }
}

ClassicProfileEngine.RAW_PROFILES = PROFILES;

module.exports = ClassicProfileEngine;
